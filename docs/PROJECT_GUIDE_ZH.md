...
因此，当前 `path_generator.py` 若已设置为 `angle=15.0`，不能直接执行生成器覆盖路径库；建议改为 `angle=11.0`，重新生成后再复核。若只追求更大安全余量，使用 `angle=10.0`；若后续实测显示轮胎侧滑或控制跟踪误差较大，应进一步降低 `angle` 或提高 `minPathScale`。
